...
$$
We are also interested in the effect of each treatment in $\{B, C, D, E\}$ (relative to $A$) within the sub-population defined by $X_{12} \in \{0,1\}.$
$$
\textrm{GATE}(z,x) = \mathbb{E}[Y(z) - Y(A) \vert X_{12} = x]
$$
...
Specifically, we wish to compute
  ...
  * For each value of $X_{12} \in \{0,1\},$ $\textrm{argmax}_{z} \textrm{GATE}(z,x).$
