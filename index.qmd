...
$$
\textrm{subCATE}(z,x) = \left[\sum_{i = 1}^{n}{\mathbf{1}(x_{i,12}= x)}\right]^{-1} \times \sum_{i=1}^{n}{\left[\mathbb{E}[Y_{i}(z) - Y_{i}(a) \vert \boldsymbol{\mathbf{X}} = \boldsymbol{\mathbf{x}}_{i}] \times \mathbf{1}(x_{i,12} = x)\right]}.
$$
...
Please create a single compressed `.zip` or `.tar.gz` archive containing all your submission files, a README file containing a short, narrative description of your solution, and reproducible code. 
...
Your compressed archive should named `teamID_submissionID.zip` or `teamID_submission_ID.tar.gz`.
...
Recall that the treatment $Z$ was **randomly** assigned. So, there are no confounders. 
However, some of the $X$’s may modify the effect of the treatment relative to the business as usual/control condition.
...
No. After the submission window closes on April 20, we will evaluate each submission and report the results of the Data Challenge at the [ACIC Meeting in May 2026](https://sci-info.org/2026-meeting-2/).


## Preliminary Results

A copy of the preliminary results, which were presented at the 2026 ACIC Conference, are below.

<body>
    <iframe src="acic_presentation.pdf" width="100%" height="475px" style="position: relative;z-index: 9999999;">
    </iframe>
</body>
